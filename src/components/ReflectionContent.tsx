...
const ReflectionContent: React.FC<ReflectionContentProps> = ({
  ...
  return (
    <div ref={contentRef} className="space-y-4 sm:space-y-6 overflow-y-auto max-h-[calc(100vh-8rem)] px-2 sm:px-0">
      {/* Header Section */}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        {/* Sync Status */}
        <SyncStatusCard user={user} token={token} learnings={learnings} />
        ...
        <ActionButtons
          isSaving={isSaving}
          learmingsEmpty={learnings.trim().length === 0}
          onSkip={() => {
            setLearnings("");
            setError(null);
            setShareError(null);
